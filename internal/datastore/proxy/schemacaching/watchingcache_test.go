...
func TestOldWatchingCacheFallbackToStandardCache(t *testing.T) {
	...
	expectedKey := cache.StringKey("n:somenamespace@" + r.String())
	entry, ok := def.c.Get(expectedKey)
	require.True(t, ok)
	require.Error(t, entry.notFound)
...
func (*fakeDatastore) OptimizedRevision(context.Context) (datastore.RevisionWithSchemaHashAndValidity, error) {
	return datastore.RevisionWithSchemaHashAndValidity{}, fmt.Errorf("not implemented")
}
