...
// mockExecutor is a mock implementation of ChunkedBytesExecutor for testing.
...
func (m *mockExecutor) BeginTransaction(ctx context.Context) (ChunkedBytesTransaction, error) {
	return m.transaction, nil
}

func (m *mockExecutor) ExecuteRead(ctx context.Context, builder sq.SelectBuilder) (map[int][]byte, error) {
	if m.readErr != nil {
...
func TestWriteChunkedBytes_InsertWithTombstones(t *testing.T) {
	txn := &fakeTransaction{}
	executor := &mockExecutor{transaction: txn}
