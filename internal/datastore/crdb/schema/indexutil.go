...
func parseIndexColumns(columnsSQL string) ([]string, error) {
	...
	for i, col := range columns {
		columns[i] = regexp.MustCompile(`^\s+|\s+$`).ReplaceAllString(col, "")
	}
...
func checkIfMatchingIndex(filter datastore.RelationshipsFilter, idx common.IndexDefinition) (int, error) {
	columnNames := parsedColumnsPerIndex[idx.Name]
	lastMatchingColIndex := -1
	...
	for columnIndex, colName := range columnNames {
		...
		default:
			return -1, spiceerrors.MustBugf("unknown column filter status: %d", filterStatus)
		}
	}
