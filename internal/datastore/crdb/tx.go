...
type executeTxRetryFunc func(context.Context, *pgxpool.Pool, pgx.TxOptions, transactionFn) error

func executeWithMaxResetsAndRetries(maxRetries int) executeTxRetryFunc {
	return func(ctx context.Context, pool *pgxpool.Pool, txOptions pgx.TxOptions, fn transactionFn) (err error) {
		...
		for tries = 0; tries < maxRetries; tries++ {
			...
			var attempts int
			attempts, err = execute(ctx, txConn, txOptions, fn, maxRetries)
			txConn.Release()
			if err != nil {
				tries += attempts
			} else {
			...
			if resetable(ctx, err) {
				log.Ctx(ctx).Err(err).Msg("resettable error, reconnecting")
				resets++
				continue
			}
...
func execute(ctx context.Context, conn *pgxpool.Conn, txOptions pgx.TxOptions, fn transactionFn, maxRetries int) (tries int, err error) {
	var tx pgx.Tx
