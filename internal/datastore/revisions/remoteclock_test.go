	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, timeAndExpected := range tc.times {
				...
				optimized, _, err := rcr.OptimizedRevision(t.Context())
				require.NoError(err)
				require.True(
