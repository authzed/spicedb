	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, timeAndExpected := range tc.times {
				...
				optimizedResult, err := rcr.OptimizedRevision(t.Context())
				require.NoError(err)
				optimized := optimizedResult.Revision
				require.True(
...
func TestRemoteClockCheckRevisions(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := rcr.CheckRevision(t.Context(), testRevision)
			if tc.expectError {
				require.Error(err)
			} else {
				require.NoError(err)
