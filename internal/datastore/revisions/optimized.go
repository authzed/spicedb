...
const defaultOptimizedRevisionFallbackTimeout = 30 * time.Second
...
func (cor *CachedOptimizedRevisions) OptimizedRevision(ctx context.Context) (datastore.RevisionWithSchemaHash, error) {
	...
	// poisoned in-flight call, so a brief wedge does not fail the request. The
	// retry is naturally throttled by the connection pool's maximum size, so even
	// when many waiters retry at once the datastore is not overwhelmed.
