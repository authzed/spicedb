		...
		for rows.Next() {
			...
			if deletedTxn > afterRevision && deletedTxn <= newRevision {
				if schemaType.objectType == "namespace" {
					if err = stagedChanges.AddDeletedNamespace(ctx, revisions.NewForTransactionID(deletedTxn), objName); err != nil {
						return err
					}
				} else if schemaType.objectType == "caveat" {
					if err = stagedChanges.AddDeletedCaveat(ctx, revisions.NewForTransactionID(deletedTxn), objName); err != nil {
						return err
					}
				}
			}
