...
type pgDeleteBenchScenario struct {
	name      string
	namespace string
	// subjectID returns the subject object ID for the row at the given index on
	// the given relation.
	subjectID func(index int, relation int) string
}
...
var pgDeleteBenchScenarios = []pgDeleteBenchScenario{
	{
		...
		name:      "UniqueSubject",
		namespace: "unique",
		subjectID: func(index int, _ int) string { return "unique-" + strconv.Itoa(index) },
	},
	{
		...
		name:      "SharedSubject",
		namespace: "shared",
		subjectID: func(_ int, relation int) string { return "shared-" + strconv.Itoa(relation) },
	},
	{
		// Every relationship is granted to the wildcard subject.
		name:      "WildcardSubject",
		namespace: "wild",
		subjectID: func(int, int) string { return tuple.PublicWildcard },
	},
}
...
func BenchmarkPostgresDeleteWithLimit(b *testing.B) {
	...
	seedRelationships(ctx, b, ds, pgDeleteBenchBackgroundRows, func(index int) tuple.Relationship {
		return benchRel(
			"bg-"+strconv.Itoa(index%pgDeleteBenchBackgroundNamespaces),
			"obj-"+strconv.Itoa(index),
			"rel"+strconv.Itoa((index/pgDeleteBenchBackgroundNamespaces)%pgDeleteBenchBackgroundRelations),
			"user-"+strconv.Itoa(index%pgDeleteBenchBackgroundSubjects),
		)
	})

	for _, scenario := range pgDeleteBenchScenarios {
		seedRelationships(ctx, b, ds, pgDeleteBenchScenarioRelations*pgDeleteBenchScenarioRowsPerRelation, func(index int) tuple.Relationship {
			relation := index % pgDeleteBenchScenarioRelations
			return benchRel(
				scenario.namespace,
				"obj-"+strconv.Itoa(index),
				"rel"+strconv.Itoa(relation),
				scenario.subjectID(index, relation),
			)
		})
	}

	// Maintenance connection, used off the timer to keep the table's statistics
	// and dead-tuple count representative between iterations.
				...
				b.Run(fmt.Sprintf("limit=%d", limit), func(b *testing.B) {
					filter := &v1.RelationshipFilter{
						...
						OptionalSubjectFilter: &v1.SubjectFilter{
							SubjectType: pgDeleteBenchSubjectType,
						},
					}
...
func benchRel(resourceType, resourceID, relation, subjectID string) tuple.Relationship {
	return tuple.Relationship{
		RelationshipReference: tuple.RelationshipReference{
			...
			Subject: tuple.ObjectAndRelation{
				ObjectType: pgDeleteBenchSubjectType,
				ObjectID:   subjectID,
