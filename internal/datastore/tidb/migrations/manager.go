...
var (
	...
	// Manager is the singleton migration manager instance for TiDB
	Manager = migrate.NewManager[*TiDBDriver, Wrapper, TxWrapper]()
)

// Wrapper makes it possible to forward the table schema needed for TiDB MigrationFunc to run
type Wrapper struct {
	db     *sql.DB
	tables *tables
}
...
func registerMigration(version, replaces string, up migrate.MigrationFunc[Wrapper], upTx migrate.TxMigrationFunc[TxWrapper]) error {
	// validate migration names to ensure they are compatible with TiDB column names
	for _, v := range []string{version, replaces} {
		if match := migrationNameRe.MatchString(version); !match {
			return fmt.Errorf("migration from '%s' to '%s': '%s' is an invalid TiDB migration version, expected to match pattern '%s'",
