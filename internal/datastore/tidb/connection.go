...
var (
	connectHistogram = prometheus.NewHistogram(prometheus.HistogramOpts{
		Namespace: "spicedb",
		Subsystem: "datastore",
		Name:      "tidb_connect_duration",
		Help:      "distribution in seconds of time spent opening a new TiDB connection.",
		Buckets:   []float64{0.01, 0.1, 0.5, 1, 5, 10, 25, 60, 120},
	})
	connectCount = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: "spicedb",
		Subsystem: "datastore",
		Name:      "tidb_connect_count_total",
		Help:      "number of TiDB connections opened.",
	}, []string{"success"})
)

// instrumentedConnector wraps the default TiDB driver connector
...
func (d *instrumentedConnector) Connect(ctx context.Context) (driver.Conn, error) {
	ctx, span := tracer.Start(ctx, "openTiDBConnection")
	defer span.End()
	...
	if err != nil {
		...
		log.Ctx(ctx).Error().Err(err).Msg("failed to open tidb connection")
		return nil, fmt.Errorf("failed to open connection to tidb: %w", err)
	}
