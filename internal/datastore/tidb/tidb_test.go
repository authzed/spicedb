...
func (ds *Datastore) ExampleRetryableError() error {
	return &mysql.MySQLError{
		Number: errTiDBDeadlock,
	}
}
...
func (dst *datastoreTester) createDatastore(revisionQuantization, gcInterval, gcWindow time.Duration, _ uint16) (datastore.Datastore, error) {
	...
	ds := dst.b.NewDatastore(dst.t, func(engine, uri string) datastore.Datastore {
		ds, err := newTiDBDatastore(ctx, uri, primaryInstanceID,
...
func createDatastoreTest(b testdatastore.RunningEngineForTest, tf datastoreTestFunc, options ...Option) func(*testing.T) {
	return func(t *testing.T) {
		...
		ds := b.NewDatastore(t, func(engine, uri string) datastore.Datastore {
			ds, err := newTiDBDatastore(ctx, uri, primaryInstanceID, options...)
...
func createMultiDatastoreTest(b testdatastore.RunningEngineForTest, tf multiDatastoreTestFunc, options ...Option) func(*testing.T) {
	return func(t *testing.T) {
		...
		ds := b.NewDatastore(t, func(engine, uri string) datastore.Datastore {
			...
			ds2, err := newTiDBDatastore(ctx, uri, primaryInstanceID, options...)
			require.NoError(t, err)
...
func TestTiDBDatastoreDSNWithoutParseTime(t *testing.T) {
	_, err := NewTiDBDatastore(context.Background(), "root:password@(localhost:1234)/mysql") // URI doesn't matter much here as parsing is tested
	require.ErrorContains(t, err, "https://spicedb.dev/d/parse-time-tidb")
}

func TestTiDBDatastore(t *testing.T) {
	b := testdatastore.RunTiDBForTestingWithOptions(t, testdatastore.TiDBTesterOptions{MigrateForNewDatastore: true}, "") // Needs TiDB test runner
	...
	additionalTiDBTests(t, b)
}

func TestTiDBRevisionTimestamps(t *testing.T) {
	b := testdatastore.RunTiDBForTestingWithOptions(t, testdatastore.TiDBTesterOptions{MigrateForNewDatastore: true}, "") // Needs TiDB test runner
	t.Run("TransactionTimestamps", createDatastoreTest(b, TransactionTimestampsTest, defaultOptions...))
}

func additionalTiDBTests(t *testing.T, b testdatastore.RunningEngineForTest) {
...
func PrometheusCollectorTest(t *testing.T, ds datastore.Datastore) {
	...
	for _, metric := range metrics {
		...
		if metric.GetName() == "spicedb_datastore_tidb_connect_count_total" {
			connectorStatsFound = true
		}
	}
	req.True(collectorStatsFound, "tidb datastore did not issue prometheus metrics")
	req.True(connectorStatsFound, "tidb datastore connector did not issue prometheus metrics")
}
...
func QuantizedRevisionTest(t *testing.T, b testdatastore.RunningEngineForTest) {
	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			ds := b.NewDatastore(t, func(engine, uri string) datastore.Datastore {
				ds, err := newTiDBDatastore( // Changed here
					ctx,
					uri,
					primaryInstanceID,
			...
			tds := ds.(*Datastore) // Changed variable name for clarity

			mgg, err := tds.BuildGarbageCollector(ctx)
			require.NoError(err)
			defer mgg.Close()
			...
			tx, err := tds.db.BeginTx(ctx, nil) // Changed here
			require.NoError(err)

			if len(tc.relativeTimes) > 0 {
				bulkWrite := sb.Insert(tds.driver.RelationTupleTransaction()).Columns(colTimestamp) // Changed here
			...
			queryRevision := fmt.Sprintf(
				querySelectRevision,
				colID,
				tds.driver.RelationTupleTransaction(), // Changed here
				colTimestamp,
...
func TestTiDBMigrations(t *testing.T) {
	req := require.New(t)

	db := datastoreTiDB(t, false) // Needs TiDB test instance
	migrationDriver := migrations.NewTiDBDriverFromDB(db, "")
...
func TestTiDBMigrationsWithPrefix(t *testing.T) {
	req := require.New(t)

	prefix := "spicedb_"
	db := datastoreTiDB(t, false) // Needs TiDB test instance
	migrationDriver := migrations.NewTiDBDriverFromDB(db, prefix)
...
func TestTiDBWithAWSIAMCredentialsProvider(t *testing.T) {
	...
	_, err := NewTiDBDatastore(context.Background(), "root:password@(localhost:1234)/mysql?parseTime=True&tls=skip-verify", CredentialsProviderName("aws-iam"))
...
// datastoreTiDB provides a TiDB connection for testing migrations.
// TODO: This needs to connect to a real TiDB test instance.
func datastoreTiDB(t *testing.T, migrate bool) *sql.DB {
	var databaseURI string
	// This is the critical part that needs to change for TiDB.
	// We need a TiDB equivalent of RunMySQLForTestingWithOptions.
	// For now, this will fail or use MySQL if not adapted.
	testdatastore.RunTiDBForTestingWithOptions(t, testdatastore.TiDBTesterOptions{MigrateForNewDatastore: migrate}, "").NewDatastore(t, func(engine, uri string) datastore.Datastore {
		databaseURI = uri
		return nil
	})

	// The driver name "mysql" is used for TiDB as well with go-sql-driver.
