...
func runPGServer(t *testing.T, client *authzed.Client) {
	...
	go func() {
		_ = pgserver.Run(ctx, "localhost:5433")
	}()
...
func runSpiceDB(t *testing.T) *authzed.Client {
	...
	for retryCount < 10 {
		...
		if schemaErr != nil && strings.Contains(schemaErr.Error(), "NotFound") {
	...
	t.Fatalf("failed to connect to SpiceDB server after %d attempts", retryCount)
	return nil
}
