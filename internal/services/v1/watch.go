...
func (ws *watchServer) Watch(req *v1.WatchRequest, stream v1.WatchService_WatchServer) error {
	...
	var currentSchemaHash datalayer.SchemaHash
	if req.OptionalStartCursor != nil && req.OptionalStartCursor.Token != "" {
		decoded, err := zedtoken.DecodeRevision(req.OptionalStartCursor, dl)
		if err != nil {
		...
		currentSchemaHash = decoded.SchemaHash
	} else {
		var err error
		afterRevision, currentSchemaHash, err = dl.OptimizedRevision(ctx)
		if err != nil {
		...
		case update, ok := <-updates:
			if ok {
				...
				if len(update.ChangedDefinitions) > 0 || len(update.DeletedCaveats) > 0 || len(update.DeletedNamespaces) > 0 {
					// If there are schema changes, we simply mark the schema hash as unknown, since we don't want to attempt to track it across changes.
					currentSchemaHash = datalayer.NoSchemaHashForWatch
				}

				filteredRelationshipUpdates := filterRelationshipUpdates(objectTypes, filters, update.RelationshipChanges)
				zedToken, err := zedtoken.NewFromRevision(ctx, update.Revision, currentSchemaHash, dl)
				if err != nil {
					return err
				}
