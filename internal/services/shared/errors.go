...
func RewriteError(ctx context.Context, err error, config *ConfigForErrors) error {
	...
	case errors.As(err, &datastore.ErrFilterAlreadyRegistered{}):
		return spiceerrors.WithCodeAndReason(err, codes.FailedPrecondition, v1.ErrorReason_ERROR_REASON_FILTER_ALREADY_REGISTERED)
	case errors.As(err, &datastore.ErrFilterNotRegistered{}):
		return spiceerrors.WithCodeAndReason(err, codes.FailedPrecondition, v1.ErrorReason_ERROR_REASON_FILTER_NOT_REGISTERED)
