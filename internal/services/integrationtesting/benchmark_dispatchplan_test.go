...
// dispatchPlanHandle holds the precompiled state needed to run dispatch plan benchmarks.
type dispatchPlanHandle struct {
	ds       datastore.Datastore
	revision datastore.Revision
	schema   *schema.Schema
}

func newDispatchPlanHandle(b *testing.B, fileName string) *dispatchPlanHandle {
	b.Helper()
	...
	return &dispatchPlanHandle{ds: ds, revision: rev, schema: fullSchema}
}

// compileIterator builds and compiles an iterator for the given resource type and permission.
func (h *dispatchPlanHandle) compileIterator(b *testing.B, resourceType, permission, subjectType, subjectRelation string) query.Iterator {
	b.Helper()
...
func (h *dispatchPlanHandle) newLocalContext(ctx context.Context) *query.Context {
...
// localPlanDispatcher that handles DispatchQueryPlan by compiling and executing the plan locally.
func (h *dispatchPlanHandle) newDispatchContext(ctx context.Context) *query.Context {
	planCtx := dispatch.NewPlanContext(h.revision.String(), nil, 0, 0)
	lpd := &localPlanDispatcher{handle: h}
...
// a caching layer wrapping the localPlanDispatcher. This exercises the Check cache path.
func (h *dispatchPlanHandle) newCachedDispatchContext(b *testing.B, ctx context.Context, cachingDispatcher *caching.Dispatcher) *query.Context {
	planCtx := dispatch.NewPlanContext(h.revision.String(), nil, 0, 0)
...
// newCachingDispatcher creates a caching dispatcher wrapping a localPlanDispatcher.
func (h *dispatchPlanHandle) newCachingDispatcher(b *testing.B) *caching.Dispatcher {
	b.Helper()
...
// localPlanDispatcher is a minimal dispatcher that handles DispatchQueryPlan by compiling
// the plan from schema and executing it locally. This simulates what the real
// localDispatcher will do in Phase 4.
type localPlanDispatcher struct {
	handle *dispatchPlanHandle
}

func (d *localPlanDispatcher) DispatchCheck(_ context.Context, _ *v1.DispatchCheckRequest) (*v1.DispatchCheckResponse, error) {
	return nil, fmt.Errorf("not implemented")
}
func (d *localPlanDispatcher) DispatchExpand(_ context.Context, _ *v1.DispatchExpandRequest) (*v1.DispatchExpandResponse, error) {
	return nil, fmt.Errorf("not implemented")
}
func (d *localPlanDispatcher) DispatchLookupResources2(_ *v1.DispatchLookupResources2Request, _ dispatch.LookupResources2Stream) error {
	return fmt.Errorf("not implemented")
}
func (d *localPlanDispatcher) DispatchLookupResources3(_ *v1.DispatchLookupResources3Request, _ dispatch.LookupResources3Stream) error {
	return fmt.Errorf("not implemented")
}
func (d *localPlanDispatcher) DispatchLookupSubjects(_ *v1.DispatchLookupSubjectsRequest, _ dispatch.LookupSubjectsStream) error {
	return fmt.Errorf("not implemented")
}
func (d *localPlanDispatcher) Close() error              { return nil }
func (d *localPlanDispatcher) ReadyState() dispatch.ReadyState { return dispatch.ReadyState{IsReady: true} }

func (d *localPlanDispatcher) DispatchQueryPlan(req *v1.DispatchQueryPlanRequest, stream dispatch.PlanStream) error {
	ctx := stream.Context()
	...
	lpd := &localPlanDispatcher{handle: d.handle}
	executor := dispatch.NewDispatchExecutor(lpd, planCtx)
...
func (d *localPlanDispatcher) findIterator(req *v1.DispatchQueryPlanRequest) (query.Iterator, error) {
...
func BenchmarkDispatchPlanCheck(b *testing.B) {
	tests := []struct {
		name                                                 string
		file                                                 string
	...
	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			handle := newDispatchPlanHandle(b, tt.file)
			...
			b.Run("cached_dispatch_executor", func(b *testing.B) {
				...
				for b.Loop() {
					qctx := handle.newCachedDispatchContext(b, b.Context(), cd)
					path, err := qctx.Check(it.Clone(), resource, subject)
					require.NoError(b, err)
					require.NotNil(b, path)
				}
...
func BenchmarkDispatchPlanLookupResources(b *testing.B) {
	tests := []struct {
		name                                    string
		file                                    string
...
func BenchmarkDispatchPlanLookupSubjects(b *testing.B) {
	tests := []struct {
		name                                     string
		file                                     string
		resourceType, resourceID, permission     string
