...
func TestNoNewEnemy(t *testing.T) {
	...
	batchSize := 100
	testCount := 1000
	excludes, directs := generateTuples(fillerCount)
	...
	for i := 0; i < testCount; i++ {
		...
		r1, err := client.Write(testCtx, &v0.WriteRequest{
			Updates: []*v0.RelationTupleUpdate{{Operation: v0.RelationTupleUpdate_TOUCH, Tuple: tupleExclude}},
		})
		if err != nil {
			fmt.Println(err)
			continue
		...
		r2, err := client2.Write(testCtx, &v0.WriteRequest{
			Updates: []*v0.RelationTupleUpdate{{Operation: v0.RelationTupleUpdate_TOUCH, Tuple: tupleDirect}},
		})
		if err != nil {
			fmt.Println(err)
			continue
		...
		canHas, err := client2.Check(context.Background(), &v0.CheckRequest{
