...
### Fixed
- MemDB: the fallback added in #3366 advertised head alongside the validity of the quantized revision it had just discarded. Optimized revisions are cached for as long as they are valid, so that let a caller hold head for the rest of the quantization interval and miss every write made during it — the staleness the fallback exists to prevent. MemDB now reports no validity when it substitutes head, so each request derives it afresh. (https://github.com/authzed/spicedb/pull/3374)
