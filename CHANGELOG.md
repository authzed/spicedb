...
- `spicedb datastore gc` and `spicedb datastore repair` failed with `datastore of type 'postgres' does not support garbage collection` (or `... the repair operation`) whenever a read replica was configured, even though the primary supports both. Optional datastore capabilities are found by walking the proxy chain, and the read-replica proxies did not expose the link to the primary, so the walk stopped at them. The same gap hid optional capabilities behind the schema definition cache, which is the outermost datastore in the default server configuration. Both now expose the link; the replica proxies deliberately expose only the primary, since these operations belong to it.
