...
### Changed
- Caches: the default dispatch cache budgets have been lowered — `--dispatch-cache-max-cost` from `30%` to `20%`, and `--dispatch-cluster-cache-max-cost` from `70%` to `45%`. Both are percentages of available memory, which is 75% of the process memory limit; the remaining 25% is headroom for memory the caches do not account for (goroutine stacks, GC metadata, fragmentation, per-entry overhead inside the cache). At 30% + 70% the two dispatch caches asked for exactly 100% of available memory, leaving none of that headroom for what it is meant to cover. Together they now ask for 65% of it, about 49% of the memory limit rather than 75%; the ratio between the two caches is unchanged. At a 1GiB memory limit the pair goes from 230MiB + 538MiB to 154MiB + 346MiB, and at 8GiB from 1843MiB + 4301MiB to 1229MiB + 2765MiB. Deployments running on the defaults get smaller dispatch caches and, depending on working-set size, potentially lower hit rates; deployments that set either flag explicitly are unaffected. This reduces over-allocation rather than eliminating it, and it matters because https://github.com/authzed/spicedb/pull/3370 fixes cache entries being accounted at roughly a quarter of the memory they actually use — until that lands the caches never grew into the budget they were given. (https://github.com/authzed/spicedb/pull/3373)
