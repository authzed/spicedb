...
### Changed
- Startup: SpiceDB now waits for its Postgres and CockroachDB connection pools to reach the configured minimum number of connections (`--datastore-conn-pool-read-min-open` and `--datastore-conn-pool-write-min-open`) before it finishes starting and reports itself ready. Previously the pools were filled in the background, so a pod could be marked Ready by Kubernetes with empty pools and the first burst of traffic after a rollout had to open every connection it used, showing up as a latency spike or as errors. If a pool cannot be filled, startup now fails with an error naming the pool and the number of connections it was short of, instead of starting an instance that cannot serve the load it was configured for. Note for anyone embedding SpiceDB as a library: a datastore constructed with no connection-pool options at all now keeps a minimum of one connection rather than defaulting its minimum to pgx's own maximum, so it no longer establishes a core count's worth of connections before it can be used. Running SpiceDB is unaffected, as it always configures both bounds.
