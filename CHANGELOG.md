...
### Fixed
- WriteRelationships: a caveat context that does not match the caveat's parameters (a value of the wrong type, or an unknown parameter) is now rejected with `InvalidArgument` and the `ERROR_REASON_CAVEAT_PARAMETER_TYPE_ERROR` reason, the same way `CheckPermission` reports a bad context. It was previously returned as `Unknown` with no error reason. (https://github.com/authzed/spicedb/pull/3378)
