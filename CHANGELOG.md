...
- CockroachDB: the expiration migration now sets the `ttl_disable_changefeed_replication` storage parameter on the relationship tables as part of the row-level TTL policy (on CockroachDB ≥ 24.1), so a freshly migrated database already suppresses TTL deletes from the Watch API changefeed. Previously the parameter was only ever set by an `ALTER TABLE` issued on every datastore construction, which meant a database whose SpiceDB credentials lack `ALTER TABLE` never got it at all. That startup check remains, and is still the only thing that sets the parameter on databases migrated before this change. (https://github.com/authzed/spicedb/pull/3336)
