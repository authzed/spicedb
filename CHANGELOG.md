...
### Fixed
- MemDB: requests made shortly after startup could fail with `object definition not found`, or return no results, even though the schema and relationships had already been written. Reads at `minimize_latency` (the default) use a revision rounded down to the nearest quantization interval, and when that boundary happened to fall between the moment the datastore was created and the first write to it, the read was served from the empty snapshot taken at creation. This was most visible when starting SpiceDB with `--datastore-engine=memory` and `--datastore-bootstrap-files`, where the window lasted until the next interval boundary (up to 5 seconds by default). MemDB now advertises its head revision in that case, so committed data is never hidden behind the datastore's own creation. (https://github.com/authzed/spicedb/pull/3366)
