...
- Spanner: Watch now sends checkpoints on a quiet database. Checkpoints were only produced as a side effect of a relationship or schema change arriving, so a consumer watching a database with little or no write traffic received none at all and could never move its cursor forward — the opposite of what a checkpoint is for. Spanner's own idle signal, the change stream heartbeat, was being read and thrown away; it is now used. Checkpoints are also now held back to the least advanced part of the change stream. Spanner reads a change stream in several partitions at once, and a checkpoint was previously sent at the timestamp of whichever partition had just delivered something, which told the consumer it had seen everything up to that point while changes of the same age were still in flight on another partition. A consumer that trusted the checkpoint and moved its cursor forward would skip those changes. (https://github.com/authzed/spicedb/pull/3362)
