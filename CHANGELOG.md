...
- CockroachDB: writes no longer fail with `ResourceExhausted` while the connection pool is still opening its connections. SpiceDB opens connections to CockroachDB no faster than one per `--datastore-connect-rate`, 100ms by default, so a pool takes about a second to fill — and throughout that window `--write-conn-acquisition-timeout`, 30ms by default, was rejecting writes with "consider increasing write pool size and/or datastore capacity". That advice did not apply: the pool was not out of connections, it had not finished opening them. It affected the first writes after a restart or a scale-up, writes while the pool grows from its configured minimum towards its maximum, and writes while the pool replaces the connections it had to a CockroachDB node that went unhealthy. That timeout is backpressure for a pool whose connections are all in use, so it now applies only to that case, and a pool that is still opening connections is given as long as opening them can take. A pool with every connection it is allowed handed out still fails fast, unchanged. (https://github.com/authzed/spicedb/pull/3363)
