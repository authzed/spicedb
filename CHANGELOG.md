...
### Fixed
- Fixed a nil pointer dereference panic in `CheckBulkPermissions`. The dispatch key does not depend on the debug flag, so under concurrent load a tracing-enabled check could share a singleflight dispatch with a non-tracing bulk check and leak its debug trace into the non-tracing caller; that trace contains combined nodes with a nil `Check.Request`, which panicked the resource-ID matching loop. Debug-enabled checks are no longer singleflighted together with non-debug checks (which also prevents a tracing check from silently losing its trace to a non-debug leader), and the bulk loop guards against nil entries as defense-in-depth. (https://github.com/authzed/spicedb/pull/3174)
