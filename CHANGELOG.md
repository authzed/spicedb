...
### Fixed
- Caches: cached entries were accounted at their payload plus key bytes only, leaving out the ~150 bytes of fixed per-entry overhead the cache itself holds (the entry node, its hash-table slot, frequency-sketch state, the key's backing allocation and the boxing of the value). That overhead does not shrink with the entry, so for the small entries a `CheckPermission` produces it was most of the entry: real memory was up to 4.4x the cost the cache charged itself. A cache therefore reported itself a quarter full while actually being full and never evicted, and with cluster dispatch enabled the two dispatch caches together could hold roughly three times `GOMEMLIMIT` of real heap. The symptom was not a clean out-of-memory: the memory-protection middleware shed valid requests with `ResourceExhausted` while the cache still believed it had room, with relief coming only from TTL expiry. **This is a visible behaviour change: at the same configured budget the caches now hold fewer entries.** Relative to before, entries retained are about 24% for single checks over short object IDs (4.2x fewer), 33% over UUID object IDs, 51% for `CheckBulkPermissions`, and essentially unchanged for `LookupResources`/`LookupSubjects`, whose entries are large enough that the per-entry overhead is negligible. Deployments whose caches never reach their budget see no change at all, since no eviction happens either way — that is most deployments at the default quantization window. Deployments that were relying on the over-filled cache can raise `--dispatch-cache-max-cost` and `--dispatch-cluster-cache-max-cost` explicitly, keeping in mind that the budget now means what it says. (https://github.com/authzed/spicedb/pull/3370)
...
[Unreleased]: https://github.com/authzed/spicedb/compare/v1.56.2...HEAD
[1.56.2]: https://github.com/authzed/spicedb/compare/v1.56.1...v1.56.2
