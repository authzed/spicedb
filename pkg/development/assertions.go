...
func RunAllAssertions(ctx context.Context, devContext *DevContext, assertions *blocks.Assertions) (*DeveloperErrors, error) {
	trueFailures, err := runAssertions(ctx, devContext, assertions.AssertTrue, true, "Expected relation or permission %s to exist")
	if err != nil {
		return nil, err
	}

	falseFailures, err := runAssertions(ctx, devContext, assertions.AssertFalse, false, "Expected relation or permission %s to not exist")
	if err != nil {
		return nil, err
	}
...
func runAssertions(ctx context.Context, devContext *DevContext, assertions []blocks.Assertion, expected bool, fmtString string) ([]*v0.DeveloperError, error) {
	var failures []*v0.DeveloperError
	for _, assertion := range assertions {
		tpl := tuple.MustFromRelationship(assertion.Relationship)
		cr, err := RunCheck(ctx, devContext, tpl.ObjectAndRelation, tpl.User.GetUserset())
		if err != nil {
