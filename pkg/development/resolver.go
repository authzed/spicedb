...
func (r *Resolver) ReferenceAtPosition(source input.Source, position input.Position) (*SchemaReference, error) {
	...
	relationReference := func(relation *core.Relation, def *schema.Definition) (*SchemaReference, error) {
		...
		lineNumber, err := safecast.ToInt(relation.GetSourcePosition().GetZeroIndexedLineNumber())
		if err != nil {
			log.Err(err).Msg("could not cast lineNumber to uint32")
		}
		columnPosition, err := safecast.ToInt(relation.GetSourcePosition().GetZeroIndexedColumnPosition())
		if err != nil {
	...
	if ts, relation, ok := r.typeReferenceChain(nodeChain); ok {
		...
		// NOTE: zeroes are fine here to mean "unknown"
		lineNumber, err := safecast.ToInt(def.GetSourcePosition().GetZeroIndexedLineNumber())
		if err != nil {
			log.Err(err).Msg("could not cast lineNumber to uint32")
		}
		columnPosition, err := safecast.ToInt(def.GetSourcePosition().GetZeroIndexedColumnPosition())
		if err != nil {
			log.Err(err).Msg("could not cast columnPosition to uint32")
		}
	...
	if caveatDef, ok := r.caveatTypeReferenceChain(nodeChain); ok {
		// NOTE: zeroes are fine here to mean "unknown"
		lineNumber, err := safecast.ToInt(caveatDef.GetSourcePosition().GetZeroIndexedLineNumber())
		if err != nil {
			log.Err(err).Msg("could not cast lineNumber to uint32")
		}
		columnPosition, err := safecast.ToInt(caveatDef.GetSourcePosition().GetZeroIndexedColumnPosition())
		if err != nil {
			log.Err(err).Msg("could not cast columnPosition to uint32")
		}
