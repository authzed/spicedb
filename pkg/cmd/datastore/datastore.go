...
type Config struct {
	...
	SSLMode  string `debugmap:"visible"`
...
func RegisterDatastoreFlagsWithPrefix(flagSet *pflag.FlagSet, prefix string, opts *Config) error {
	...
	// Granular connection parameters (alternative to datastore-conn-uri)
...
// Returns an empty string if granular parameters are not provided.
func buildConnectionURI(engine, host, port, username, password, database, sslMode string) string {
	// Only build URI if at least host is provided
	if host == "" {
		return ""
	}
	...
	case PostgresEngine, CockroachEngine:
		...
		// Add SSL mode parameter for postgres/cockroach
		if sslMode != "" {
			if database != "" {
				uri += "?sslmode=" + sslMode
			} else {
...
func NewDatastore(ctx context.Context, options ...ConfigOption) (datastore.Datastore, error) {
	...
	if opts.URI == "" && opts.Host != "" {
		opts.URI = buildConnectionURI(opts.Engine, opts.Host, opts.Port, opts.Username, opts.Password, opts.Database, opts.SSLMode)
		if opts.URI == "" {
