...
func newStoredSchemaReaderAdapter(reader storedSchemaReader, schemaHash SchemaHash, lastWrittenRevision datastore.Revision, cache storedSchemaCache,
) (*storedSchemaReaderAdapter, error) {
	storedSchema, err := cache.GetOrLoad(context.Background(), lastWrittenRevision, schemaHash, func(ctx context.Context) (*datastore.ReadOnlyStoredSchema, error) {
...
func WriteSchemaViaStoredSchema(ctx context.Context, rwt datastore.ReadWriteTransaction,
	...
	if v1 := storedSchema.GetV1(); v1 != nil && v1.SchemaHash != "" {
		if err := cache.Set(datastore.NoRevision, SchemaHash(v1.SchemaHash), datastore.NewReadOnlyStoredSchema(storedSchema)); err != nil {
			return err
		}
	}
