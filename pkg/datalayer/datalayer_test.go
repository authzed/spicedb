...
func testSchemaDefinitions(t *testing.T) ([]datastore.SchemaDefinition, string) {
	t.Helper()
	schemaText, _, err := generator.GenerateSchema(testDefinitions)
	require.NoError(t, err)
...
func TestComputeSchemaHash(t *testing.T) {
	...
	toCompilerDefs := func(defs []datastore.SchemaDefinition) []compiler.SchemaDefinition {
...
func TestWriteSchemaDeletesRemovedDefinitions(t *testing.T) {
	...
	for _, tc := range modes {
		t.Run(tc.name, func(t *testing.T) {
			...
			smallSchemaText, _, err := generator.GenerateSchema([]compiler.SchemaDefinition{ns.Namespace("user")})
			require.NoError(err)
...
func TestHashCacheIntegration_WriteUpdatesCache(t *testing.T) {
	...
	schemaText2, _, err := generator.GenerateSchema([]compiler.SchemaDefinition{ns.Namespace("newtype")})
	require.NoError(err)
