...
type storedSchemaCache interface {
	...
	Set(rev datastore.Revision, schemaHash SchemaHash, schema *datastore.ReadOnlyStoredSchema) error
}
...
func (noopSchemaCache) Set(_ datastore.Revision, _ SchemaHash, _ *datastore.ReadOnlyStoredSchema) error {
	return nil
}
...
func (d *defaultDataLayer) OptimizedRevision(ctx context.Context) (datastore.Revision, SchemaHash, error) {
	rev, err := d.ds.OptimizedRevision(ctx)
	if err != nil {
		return datastore.NoRevision, NoSchemaHashInLegacyMode, err
	}

	// TODO: track schema hash from watch cache or initial read for unified modes
...
func (r *revisionedReader) ReadSchema(_ context.Context) (SchemaReader, error) {
...
func (t *readWriteTransaction) ReadSchema(_ context.Context) (SchemaReader, error) {
	if t.schemaMode.ReadsFromNew() {
		return newStoredSchemaReaderAdapter(t.rwt, NoSchemaHashInTransaction, datastore.NoRevision, t.cache)
	}
...
func (t *readWriteTransaction) WriteSchema(ctx context.Context, definitions []datastore.SchemaDefinition, schemaString string, caveatTypeSet *caveattypes.TypeSet) error {
	...
	if t.schemaMode.WritesToLegacy() {
		if err := writeSchemaViaLegacy(ctx, t.rwt, t.rwt, definitions, schemaString, caveatTypeSet); err != nil {
			return err
		}
	}
...
func (r *readOnlyDatastoreAdapter) OptimizedRevision(ctx context.Context) (datastore.Revision, SchemaHash, error) {
	rev, err := r.ds.OptimizedRevision(ctx)
	if err != nil {
		return datastore.NoRevision, NoSchemaHashInLegacyMode, err
	}
	return rev, NoSchemaHashInLegacyMode, nil
