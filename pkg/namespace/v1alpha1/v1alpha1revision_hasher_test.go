...
func TestHashingSame(t *testing.T) {
	...
	hash, err := rev.ComputeHashForRevision(encoded, decoder)
	require.NoError(t, err)

	hash2, err := rev.ComputeHashForRevision(encoded, decoder)
	require.NoError(t, err)
...
func TestHashingDifferent(t *testing.T) {
	...
	hash2, err := rev.ComputeHashForRevision(encoded2, decoder)
	require.NoError(t, err)
...
func TestHashingStable(t *testing.T) {
	...
	for index, tc := range testCases {
		t.Run(fmt.Sprintf("%d", index), func(t *testing.T) {
			encoded, err := rev.ComputeV1Alpha1Revision(tc.input)
			require.NoError(t, err)
