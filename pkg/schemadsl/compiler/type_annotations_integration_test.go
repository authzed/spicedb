	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.shouldContainMetadata {
				...
				require.Empty(t, foundMetadata.TypeAnnotations, "Type annotations should be empty for permission without type annotations")
				
				// Test the helper function for retrieving type annotations
				retrievedAnnotations, err := getTypeAnnotationsFromMetadata(foundPermission)
				require.NoError(t, err)
			...
			require.Equal(t, tt.expectedAnnotations, foundMetadata.TypeAnnotations)
...
func TestTypeAnnotationsHelperFunctions(t *testing.T) {
		...
		},
		{
			...
			setupMetadata: func() *core.Metadata {
				relationMetadata := &implv1.RelationMetadata{
					...
					TypeAnnotations: []string{"ignored"},
				}
		...
		},
		{
			...
			setupMetadata: func() *core.Metadata {
				relationMetadata := &implv1.RelationMetadata{
					...
					TypeAnnotations: []string{"user", "admin"},
				}
				relAny, _ := anypb.New(relationMetadata)
				return &core.Metadata{
					MetadataMessage: []*anypb.Any{relAny},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			annotations, err := getTypeAnnotationsFromMetadata(relation)
			if tt.expectError {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
