...
func (b *outlineBuilder) buildOutlineFromOperation(p *schema.Permission, op schema.Operation) (Outline, error) {
	...
	case *schema.ArrowReference:
		...
		if !ok {
			return Outline{}, fmt.Errorf("BuildOutlineFromSchema: couldn't find left-hand relation for arrow `%s->%s` for permission `%s` in definition `%s`", perm.Left(), perm.Right(), p.Name(), p.Parent().Name())
		}
	...
	case *schema.SelfReference:
		return Outline{
			...
			Args: &IteratorArgs{
				RelationName:   p.Name(),
				DefinitionName: p.Parent().Name(),
			},
		}, nil

	case *schema.RelationReference:
		return b.buildOutlineFromSchemaInternal(p.Parent().Name(), perm.RelationName(), true)
	...
	case *schema.FunctionedArrowReference:
		rel, ok := p.Parent().GetRelation(perm.Left())
		if !ok {
			return Outline{}, fmt.Errorf("BuildOutlineFromSchema: couldn't find arrow relation `%s` for functioned arrow `%s.%s(%s)` for permission `%s` in definition `%s`", perm.Left(), perm.Left(), functionTypeString(perm.Function()), perm.Right(), p.Name(), p.Parent().Name())
		}
