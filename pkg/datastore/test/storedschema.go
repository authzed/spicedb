...
func StoredSchemaRevisionTest(t *testing.T, tester DatastoreTester) {
	...
	firstText, _, err := generator.GenerateSchema(firstDefs)
	require.NoError(err)
	...
	secondText, _, err := generator.GenerateSchema(secondDefs)
	require.NoError(err)
	...
	thirdText, _, err := generator.GenerateSchema(thirdDefs)
	require.NoError(err)
...
func StoredSchemaUpdateTest(t *testing.T, tester DatastoreTester) {
	...
	initialText, _, err := generator.GenerateSchema(initialDefs)
	require.NoError(err)
	...
	updatedText, _, err := generator.GenerateSchema(updatedDefs)
	require.NoError(err)
...
func StoredSchemaMultipleRevisionsTest(t *testing.T, tester DatastoreTester) {
	...
	for i := range numVersions {
		...
		schemaText, _, err := generator.GenerateSchema(defs)
		require.NoError(err)
...
definition middle {
	...
	sortedText, _, err := generator.GenerateSchema(sortedDefs)
	require.NoError(err)
	...
	reSortedText, _, err := generator.GenerateSchema(reSortedDefs)
	require.NoError(err)
...
func verifySchemaTypes(t *testing.T, ctx context.Context, dl datalayer.DataLayer, rev datastore.Revision, expectedTypes []string, expectedCaveats []string) {
	...
	schemaReader, err := reader.ReadSchema(t.Context())
	require.NoError(t, err)
