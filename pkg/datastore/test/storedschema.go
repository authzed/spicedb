...
func writeSchema(ctx context.Context, t *testing.T, ds datastore.Datastore,
	...
	rev, err := ds.ReadWriteTx(ctx, func(ctx context.Context, rwt datastore.ReadWriteTransaction) error {
		_, _, err := datalayer.WriteSchemaViaStoredSchema(ctx, rwt, toSchemaDefinitions(definitions), schemaText)
		return err
	})
...
func StoredSchemaReadWithinTransactionTest(t *testing.T, tester DatastoreTester) {
	...
	_, err = ds.ReadWriteTx(ctx, func(ctx context.Context, rwt datastore.ReadWriteTransaction) error {
		...
		if _, _, err := datalayer.WriteSchemaViaStoredSchema(ctx, rwt, toSchemaDefinitions(defs), schemaText); err != nil {
			return err
		}
...
func StoredSchemaConcurrentSchemaWinsTest(t *testing.T, tester DatastoreTester) {
	...
	defer schemaCancel()
	var schemaWriteErr error
	wg.Go(func() {
		_, schemaWriteErr = ds.ReadWriteTx(schemaCtx, func(ctx context.Context, rwt datastore.ReadWriteTransaction) error {
	...
	// Release the rel write, then wait for both goroutines to complete.
	close(releaseRelWrite)
	wg.Wait()
	req.NoError(schemaWriteErr, "schema write should succeed")
