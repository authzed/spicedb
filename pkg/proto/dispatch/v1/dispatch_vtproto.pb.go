...
func (m *ResourceCheckResult) CloneVT() *ResourceCheckResult {
	...
	if rhs := m.CaveatEvalInfo; rhs != nil {
		tmpContainer := make([]*CaveatEvalResult, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.CaveatEvalInfo = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *CaveatEvalResult) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (this *ResourceCheckResult) EqualVT(that *ResourceCheckResult) bool {
	...
	if len(this.CaveatEvalInfo) != len(that.CaveatEvalInfo) {
		return false
	}
	for i, vx := range this.CaveatEvalInfo {
		vy := that.CaveatEvalInfo[i]
		if p, q := vx, vy; p != q {
			if p == nil {
				p = &CaveatEvalResult{}
			}
...
func (this *CaveatEvalResult) EqualVT(that *CaveatEvalResult) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	...
	if this.ExpressionString != that.ExpressionString {
		return false
	}
...
func (this *CaveatEvalResult) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*CaveatEvalResult)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (m *ResourceCheckResult) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.CaveatEvalInfo) > 0 {
		for iNdEx := len(m.CaveatEvalInfo) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.CaveatEvalInfo[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *CaveatEvalResult) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *CaveatEvalResult) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *CaveatEvalResult) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.ExpressionString) > 0 {
		i -= len(m.ExpressionString)
		copy(dAtA[i:], m.ExpressionString)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ExpressionString)))
		i--
		dAtA[i] = 0x22
	}
	if m.Context != nil {
...
func (m *ResourceCheckResult) SizeVT() (n int) {
	...
	if len(m.CaveatEvalInfo) > 0 {
		for _, e := range m.CaveatEvalInfo {
			l = e.SizeVT()
			n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
...
func (m *CaveatEvalResult) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *ResourceCheckResult) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field CaveatEvalInfo", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.CaveatEvalInfo = append(m.CaveatEvalInfo, &CaveatEvalResult{})
			if err := m.CaveatEvalInfo[len(m.CaveatEvalInfo)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
...
func (m *CaveatEvalResult) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CaveatEvalResult: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CaveatEvalResult: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Result |= CaveatEvalResult_Result(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ExpressionString", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ExpressionString = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
