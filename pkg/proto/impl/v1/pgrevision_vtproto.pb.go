...
func (m *PostgresRevision) CloneGenericVT() proto.Message {
	return m.CloneVT()
}
...
func (m *PostgresRevision) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.RelativeXmax != 0 {
		i = encodeVarint(dAtA, i, uint64(m.RelativeXmax))
		i--
		dAtA[i] = 0x10
	}
	if m.Xmin != 0 {
		i = encodeVarint(dAtA, i, uint64(m.Xmin))
		i--
		dAtA[i] = 0x8
	}
...
func (m *PostgresRevision) SizeVT() (n int) {
	...
	_ = l
	if m.Xmin != 0 {
		n += 1 + sov(uint64(m.Xmin))
	}
	if m.RelativeXmax != 0 {
		n += 1 + sov(uint64(m.RelativeXmax))
	}
	if len(m.RelativeXips) > 0 {
		l = 0
