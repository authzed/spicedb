...
func TestEncodeDecode(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			decodedRev, _, _, err := DecodeToDispatchRevisionAndSchemaHash(context.Background(), encoded, revisions.CommonDecoder{
...
func TestDecode(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testName, func(t *testing.T) {
			...
			decodedRev, _, _, err := DecodeToDispatchRevisionAndSchemaHash(context.Background(), &v1.Cursor{
				Token: testCase.token,
